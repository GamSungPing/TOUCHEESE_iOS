...
[강건](https://github.com/kangsworkspace)
<br>

**담당 역할**<br>
- **개발**
  - Alamofire 초기 추상화 세팅
  - NavigationStack을 관리하는 NavigationManager
  - 카카오 로그인
  - 커스텀 캘린더
  - 내 정보, 상품 상세 화면, 결제 화면 UI
- **PM**
  - 스프란트 일정 관리
  - 데일리 팀장 미팅(진행상황 및 이슈 공유)
<br>

**소감**<br><br>
역대급으로 빠른 생산성을 찍어본 프로젝트!<br>
<br>
초기에 Alamofire를 세팅할 때나 몇몇 코드를 짤 때 이전에 혼자 연습한 코드를 많이 참고하여 시간을 아낄 수 있었는데, 자주 사용할만한 코드는 더 시간을 들여서 완전히 재사용 가능한 수준으로 정리해두는것이 좋을 것 같다는 생각이 들었다.<br>
<br>
주어진 시간 내에 서비스의 수요조사가 가능한 MVP 단계의 프로덕트를 만들자가 목표였기 때문에 스프린트 일정을 굉장히 타이트하게 잡았었다.<br>
그럼에도 불만없이 일정을 지켜주면서 추후 유지보수나 추가 개발이 용이한 수준으로 코드 퀄리티를 높게 유지해준 성민님께 감사했다.<br>
<br>
프로젝트를 진행하며 백엔드 분들, AOS분들, 같이 작업한 성민님을 통해 몰랐거나 부족한 부분을 많이 채울 수 있었다.<br>
다들 앞으로도 좋은일만 가득하길!🍀
